...
bool is_composite(int num) {
    if (num <= 3) return false; // Numbers 1, 2, and 3 are not composite
    for (int i = 2; i * i <= num; ++i) {
        if (num % i == 0) return true; // Found a divisor, so it's composite
    }
    return false; // No divisors found, so it's not composite
}
...
void solve() {
        ...
        if(!solved)
            ...
            ans += std::to_string(good_odd) + " ";
            ans += std::to_string(good_even) + " ";
