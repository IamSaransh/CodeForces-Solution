...
int n, m, k;
        ...
        vector<int> a(m);  // Excluded question indices
        for (int i = 0; i < m; i++) {
            cin >> a[i];
        }
        // we will use 1 based indexing for simpler code
        vector<bool> known(n + 1, false);  // Track known questions
        for (int i = 0; i < k; i++) {
            int x;
            cin >> x;
            known[x] = true;
        ...
        for (int excluded : a) {
            if(k<n-1){
                result += '0'; // knows less question than the questions in the list
                continue;
            }
            if(k==n){
            ...
            if (known[excluded]) {
                result += '0';  // Monocarp fails: the excluded question is known
            } else {
                result += '1';  // Monocarp passes: all other questions are known
            }
        }
        cout << result << "\n";
